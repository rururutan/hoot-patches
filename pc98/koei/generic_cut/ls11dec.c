/**
 *
 * LS11のデコードにはゆい氏のhoot用カッターのルーチンを流用させて頂きました
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
const UINT8	*arc;
	...
	pos = 0x110 + ((number + 1) * 12);
	if (pos > srcsize) {
		goto lzhsolve_err1;
	}
	arcsize = LOADDWORD(src + pos - 12);
	dstsize = LOADDWORD(src + pos - 8);
	arcpos = LOADDWORD(src + pos - 4);
...
int main(int argc, char **argv) {
	...
	if (argc < 3) {
		printf("usage: ls11dec [number] [filename]\n");
		return(1);
	}
