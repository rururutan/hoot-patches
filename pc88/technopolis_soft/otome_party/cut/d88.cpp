...
bool D88::SetFile(char *_fname)
{
	if (_fname == nullptr) {
		...
		m_fp = nullptr;			// ディスクイメージのファイルポインタ
	...
	for (m_numdisk = 0; m_numdisk < MAX_DISK; m_numdisk++) {	// ディスク数分だけ繰り返す
		int size;
		unsigned char header[D88_HEADER_SIZE];	// D88ヘッダ部のメモリ確保
...
bool D88::SelectDisk(int _diskno)
{
	...
	if (m_fp == nullptr) {		// D88ファイルがオープンされてないときは処理せず戻る
		return false;
	}
...
{
	if (m_fp == nullptr) {		// ファイルを開いてない場合は戻る
		return 0;
	}
	...
	for (int sec = 0;; sec++) {		// セクタ数分繰り返す
		...
		if (sec == maxsec - 1) {	// それ以上セクタがないときは中断
			printf("no sector later sec:%x maxsec:%x\n", sec, maxsec);
			break;
		}
	}
