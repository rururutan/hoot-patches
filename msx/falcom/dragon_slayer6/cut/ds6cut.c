...
/**
 * Falcom music data cutter
 *  for DS6(MSX)
 */

#define COPYRIGHT "Falcom OPLL data cut tool (" \
__DATE__ \
") (c)RuRuRu\n"
...
void write(Uint8 *_src, int _size, char *_prefix)
{
	...
	do {
		...
		while ( remain > 0 ) {
            ...
            if ( (_src[offset+size+1] == 0x00 && _src[offset+size] == 0x20) ) {
				break;
			}
		...
		printf("%s ofs:0x%04x size:0x%04x no:%02d\n", name, offset, size, count);
...
void usage()
{
	printf("usage: ds6cut <filename> <prefix>\n");
}
