// Final Fantasy I (for MSX) ファイル抽出プログラム
...
static unsigned char dir[0x200*10];				// Directory (512byte x 10sector)
...
int main(int argc, char *argv[])
{
	...
	for (int d=0; d<numdisk; d++)		// イメージ数分の処理
	{
		...
		for (fn=1; fn<sizeof(dir)/0x10; fn++)
		{
			unsigned char *ent = &dir[fn*0x10];
			if (*ent == 0x00) continue;	// 無効
			if (*ent == 0x40) continue;	// 無効
			...
			s_end  = *(ent+0x0e) + (*(ent+0x0f) << 8);	// 終了トラック
			size   = (s_end - s_trk + 1) * 0x200;		// サイズ
