...
int main(int argc, char *argv[])
{
	...
	for (int d=0; d<numdisk; d++)		// イメージ数分の処理
	{
		...
		for (fn=0; fn<sizeof(dir)/11; fn++)
		{
			unsigned char *ent = &dir[fn*11 + 1];
			if (*ent == 0xff) break;	// 終了
			if (*ent == 0x20 && *(ent+1) == 0x20) break;	// 終了
			...
			s_trk  = *(ent+0x08) + (*(ent+0x09) << 8);	// 開始セクタ
			size  = *(ent+0x0a) * 0x200; 				// サイズ
