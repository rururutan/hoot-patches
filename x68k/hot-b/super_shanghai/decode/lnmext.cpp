...
/**
 * Lenam / Super上海 decoder
 *
 * ロードスII(98)の物をbig endianにしただけ
 */
...
#define COPYRIGHT "Lenam decompress tool (" ¥
__DATE__ ¥
") (c)Fac.KUROHANE/UME-3/RuRuRu¥n"
...
void usage()
{
	printf("usage: lenamext <filename>¥n");
}
...
int main(int argc, char *argv[])
{
	...
	in = fopen(argv[1], "rb");
	if (in == NULL) {
		printf("error: can't open %s¥n", argv[1]);
		return -2;
	}
	...
	printf( "compress size: %lu(0x%04lX)¥n",
			filesize_compress, filesize_compress);

	out = fopen(tmp_name, "wb+");
	if (out == NULL) {
		printf("error: can't open tempfile¥n");
		fclose(in);
		ret = -3;
		goto error;
	}
	...
	fread(tmp, 4, 1, in);
	filesize_original = tmp[3] + (tmp[2] << 8) + (tmp[1] << 16) + (tmp[0] << 24);
	printf("original size: %lu(0x%04lX)¥n",
